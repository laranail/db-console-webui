<div>
    <flux:heading size="lg">{{ __('laranail-db-console-webui::ui.accounts') }}</flux:heading>
    ...
    @if ($generatedPassword)
        <flux:callout variant="warning" icon="key">
            <flux:callout.heading>{{ __('laranail-db-console-webui::ui.password_once') }}</flux:callout.heading>
            <flux:callout.text><code>{{ $generatedPassword }}</code></flux:callout.text>
        </flux:callout>
    @endif

    <form wire:submit="create">
        <flux:input wire:model="username" :label="__('laranail-db-console-webui::ui.username')" />
        @error('username')<flux:text variant="danger">{{ $message }}</flux:text>@enderror
        <flux:input wire:model="host" :label="__('laranail-db-console-webui::ui.host')" />
